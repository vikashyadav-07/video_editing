...
export class Store {
  ...
  async saveCanvasToVideoWithAudioWebmMp4() {
    ...
    try {
      ...
      console.error("Error playing video or starting media recorder", error);
    } finally {
      video.remove();
      audioContext.close();
    }
  ...
  refreshElements() {
    ...
    for (const element of store.editorElements) {
      switch (element.type) {
        case "video": {
          ...
          canvas.on("object:modified", (e) => {
            ...
            const newPlacement: Placement = {
              ...
              rotation: target.angle ?? placement.rotation,
              // @ts-ignore
              width: target.width ? target.width * target.scaleX : placement.width,
              // @ts-ignore
              height: target.height ? target.height * target.scaleY : placement.height,
              scaleX: 1,
              scaleY: 1,
            };
            store.updateEditorElement({ ...element, placement: newPlacement });
          });
          break;
        }
        case "image": {
          ...
          element.fabricObject = imageObject;
          element.properties.imageObject = imageObject;
          imageObject.width = imageElement.naturalWidth;
          imageObject.height = imageElement.naturalHeight;
          imageElement.width = imageElement.naturalWidth;
          imageElement.height = imageElement.naturalHeight;
          imageObject.scaleToHeight(imageElement.naturalWidth);
          imageObject.scaleToWidth(imageElement.naturalHeight);
          imageObject.scaleX = element.placement.width / imageElement.naturalWidth * element.placement.scaleX;
          imageObject.scaleY = element.placement.height / imageElement.naturalHeight * element.placement.scaleY;
          canvas.add(imageObject);
          ...
          canvas.on("object:modified", (e) => {
            ...
            let finalScale = 1;
            if (target.scaleX && target.scaleX > 0) {
              finalScale = target.scaleX / (element.placement.width / imageElement.naturalWidth);
            }
            const newPlacement: Placement = {
              ...placement,
              x: target.left ?? placement.x,
